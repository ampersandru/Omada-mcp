...
export function registerRestoreSitesFromFileServerTool(server, client) {
    ...
    server.registerTool('restoreSitesFromFileServer', {
        description: 'Restore multiple site configurations from backup files stored on an external file server (FTP/SFTP, up to 300 sites). REQUIRES explicit confirmDangerous: true — a second confirmation step is enforced because this action is irreversible.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapToolHandler('restoreSitesFromFileServer', async ({ serverConfig, siteInfos, dryRun, confirmDangerous, customHeaders }) => {
        const targetSites = siteInfos.map((s) => s.siteId).join(', ');
        if (dryRun) {
...
//# sourceMappingURL=restoreSitesFromFileServer.js.map
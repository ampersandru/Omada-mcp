...
export function registerDiagnoseClientTool(server, client) {
    server.registerTool('diagnoseClient', {
        description: "Composite client diagnostic — single call that combines a client's current connection status, detailed info (VLAN, signal, rate limit, group policy), and recent connection history (last 10 sessions). Use when troubleshooting why a device can't connect, has poor performance, or keeps disconnecting. Accepts MAC address, IP address, or hostname.",
        ...
        const resolvedMac = currentClient?.mac ?? (macPattern.test(identifier) ? identifier : undefined);
...
//# sourceMappingURL=diagnoseClient.js.map
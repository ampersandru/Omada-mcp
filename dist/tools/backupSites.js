...
export function registerBackupSitesTool(server, client) {
    ...
    server.registerTool('backupSites', {
        description: 'Trigger a multi-site configuration backup to the self/cloud server (up to 300 sites). Use getSiteBackupResult to poll the backup status for a specific site.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: false,
        },
    }, wrapMutationToolHandler('backupSites', ({ siteIds }, result, mode) => ({
        action: 'backup-sites',
        target: siteIds.join(', '),
        mode,
...
//# sourceMappingURL=backupSites.js.map
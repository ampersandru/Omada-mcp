...
export function registerListPolicyRoutesTool(server, client) {
    server.registerTool('listPolicyRoutes', {
        description: '[DEPRECATED] Use getGridPolicyRouting instead. Note: listPolicyRoutes returns an aggregated array of all policy routes, while getGridPolicyRouting is paginated and returns a single page payload (same underlying endpoint). When migrating, implement pagination to retrieve all rules. List policy routing rules for the site gateway. Policy routes direct traffic based on source IP, destination, or protocol — useful for multi-WAN environments.',
...
//# sourceMappingURL=listPolicyRoutes.js.map
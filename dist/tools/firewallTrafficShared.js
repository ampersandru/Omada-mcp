...
export function findGridRecordById(value, targetId) {
    return extractGridRecords(value).find((entry) => {
        const record = entry;
        return String(record.id ?? record.ruleId ?? record.aclId ?? '') === targetId;
    });
}
function extractLanNetworkIds(value) {
    return new Set(value
        .filter((entry) => typeof entry === 'object' && entry !== null)
        .map((entry) => entry.id)
        .filter((entry) => typeof entry === 'string' && entry.length > 0));
}
async function fetchAllApplicationIds(client, siteId, customHeaders) {
    const pageSize = 1000;
...
export async function parseAppControlPayload(client, ruleId, payload, siteId, customHeaders) {
    ...
    if (unknownApplicationIds.length > 0) {
        throw new Error(`Unknown application ids: ${unknownApplicationIds.join(', ')}. Use the documented application-control applications endpoint to discover valid applicationId values.`);
    }
    return parsedPayload;
}
export async function validateBandwidthControlPayloadReferences(client, payload, siteId, customHeaders) {
    if (payload.sourceType === 0) {
        ...
        if (unknownSourceIds.length > 0) {
            throw new Error(`Unknown LAN network ids in sourceIds: ${unknownSourceIds.join(', ')}. Use getLanNetworkList and pass each network's id field.`);
        }
    }
    const internetInfo = (await client
        .getInternetBasicPortInfo(siteId, customHeaders)
        .catch(async () => await client.getInternetInfo(siteId, customHeaders)));
    const validWanPortIds = new Set([
        ...(Array.isArray(internetInfo.portUuids) ? internetInfo.portUuids.filter((value) => typeof value === 'string') : []),
        ...
        ...(Array.isArray(internetInfo.portList)
            ? internetInfo.portList.map((entry) => entry.portId).filter((value) => typeof value === 'string' && value.length > 0)
            : []),
    ]);
    const unknownWanPortIds = payload.wanPortIds.filter((wanPortId) => !validWanPortIds.has(wanPortId));
    if (unknownWanPortIds.length > 0) {
        throw new Error(`Unknown WAN port ids: ${unknownWanPortIds.join(', ')}. Use getInternetBasicPortInfo (or getInternetInfo) and pass values from portUuids or wanPortSettings.portId.`);
    }
}
//# sourceMappingURL=firewallTrafficShared.js.map
...
export const FUTURE_CATEGORIES = new Set([
    'insights',
    'network-sim-lte',
    'account-sso',
    'hotspot-portal',
 ...
 */
export function parseToolCategories(raw) {
        ...
        if (name in CATEGORY_GROUP_ALIASES) {
            for (const cat of CATEGORY_GROUP_ALIASES[name]) {
                if (FUTURE_CATEGORIES.has(cat)) {
                    warnings.push(`OMADA_TOOL_CATEGORIES: category "${cat}" (from alias "${name}") is reserved for a future phase and has no tools yet — skipping`);
                }
                else {
...
export const DEFAULT_CAPABILITY_PROFILE = 'safe-read';
export const CAPABILITY_PROFILE_DEFAULTS = {
...
const createBooleanStringSchema = (defaultValue) => z
...
const envSchema = z
    ...
    ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Authentication credentials required: Provide either OMADA_USERNAME and OMADA_PASSWORD (for Fusion Gateway / web login) or OMADA_CLIENT_ID, OMADA_CLIENT_SECRET, and OMADA_OMADAC_ID (for OpenAPI).',
        path: ['clientId'],
    });
})
...
export function loadConfigFromEnv(env = process.env) {
    ...
    if (parsed.data.useHttp) {
        ...
        if (parsed.data.httpBindAddr && !isLoopbackBindAddress(parsed.data.httpBindAddr)) {
            warnings.push(`HTTP transport is bound to non-loopback address (${parsed.data.httpBindAddr}). Ensure access is restricted at network level.`);
        }
    }
    const effectiveAuthMode = parsed.data.authMode === 'auto' ? (parsed.data.username && parsed.data.password ? 'web' : 'openapi') : parsed.data.authMode;

...
import { dhcpReservationPayloadSchema, findLanNetworkById, findReservationByIp, findReservationByMac, getAllDhcpReservations, validateReservationIpAgainstNetwork, } from './dhcpReservationShared.js';
export function registerCreateDhcpReservationTool(server, client) {
    ...
    server.registerTool('createDhcpReservation', {
    ...
    }, wrapMutationToolHandler('createDhcpReservation', ({ mac, siteId }, result, mode) => ({
        ...
        if (reservationPayload.ip) {
            ...
            if (conflictingReservation && !reservationPayload.confirmConflict) {
                throw new Error(`IP ${reservationPayload.ip} is already reserved by ${conflictingReservation.mac}. Set confirmConflict to true only if you intend to override the controller conflict warning.`);
            }
        }
        if (dryRun) {
            return {
...
//# sourceMappingURL=createDhcpReservation.js.map
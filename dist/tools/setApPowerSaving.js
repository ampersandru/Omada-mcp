...
const inputSchema = baseInputSchema.superRefine((value, ctx) => {
    if (value.timeEnable) {
        for (const field of ['startTimeH', 'startTimeM', 'endTimeH', 'endTimeM']) {
...
export function registerSetApPowerSavingTool(server, client) {
    server.registerTool('setApPowerSaving', {
    ...
    }, wrapMutationToolHandler('setApPowerSaving', ({ apMac, siteId }, result, mode) => ({
        ...
        const current = (await client.getSitesApsPowerSaving(apMac, siteId, customHeaders));
...
//# sourceMappingURL=setApPowerSaving.js.map
...
function isGateway(device) {
    return GATEWAY_TYPES.has(device.type) || String(device.type).toLowerCase() === 'gateway';
}
...
export function registerGetGatewayHealthTool(server, client) {
    server.registerTool('getGatewayHealth', {
        description: 'Composite gateway health check — single call that auto-discovers the site gateway then retrieves its full detail (CPU, memory, firmware, ports), WAN port statuses (link state, IP, ISP), and LAN interface statuses. Use when diagnosing internet connectivity issues, WAN failover, or gateway performance problems. Optionally accepts a gatewayMac to skip discovery.',
...
//# sourceMappingURL=getGatewayHealth.js.map
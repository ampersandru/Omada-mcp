...
export function registerRestoreControllerTool(server, client) {
    ...
    server.registerTool('restoreController', {
        description: 'Restore controller configuration from a backup file stored on the self/cloud server. This will overwrite current controller settings. Use getBackupFileList to find available file names, and getRestoreResult to poll the restore status. REQUIRES explicit confirmDangerous: true — a second confirmation step is enforced because this action is irreversible.',

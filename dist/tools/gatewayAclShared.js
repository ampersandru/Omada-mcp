...
export function getAclRecordId(record) {
    return String(record['id'] ?? record['aclId'] ?? '');
}
export function findAclById(value, aclId) {
...
export function findCreatedAclByDescription(before, after, description) {
    if (!Array.isArray(after)) {
        return undefined;
    }
    const beforeIds = new Set(Array.isArray(before)
        ? before
...
async function extractSsidIds(client, siteId, customHeaders) {
    const ssidIds = new Set();
    const wlanGroups = await client.getWlanGroupList(siteId, customHeaders);
    await Promise.all(wlanGroups.map(async (group) => {
        ...
        for (const ssid of ssids) {
            const ssidId = typeof ssid === 'object' && ssid !== null && 'id' in ssid
                ? ssid.id
...
export async function validateEapAclPayloadReferences(client, payload, siteId, customHeaders) {
    ...
    if (payload.destinationType === 0 && payload.destinationIds) {
        ...
        if (unknownDestinationIds.length > 0) {
            throw new Error(`Unknown LAN network ids in destinationIds: ${unknownDestinationIds.join(', ')}. Use getLanNetworkList and pass each network's id field.`);
        }
    }
...
//# sourceMappingURL=gatewayAclShared.js.map
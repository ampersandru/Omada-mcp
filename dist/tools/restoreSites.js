...
export function registerRestoreSitesTool(server, client) {
    ...
    server.registerTool('restoreSites', {
        description: 'Restore multiple site configurations from backup files stored on the self/cloud server (up to 300 sites). Use getSiteBackupFileList to get available file names. REQUIRES explicit confirmDangerous: true — a second confirmation step is enforced because this action is irreversible.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapToolHandler('restoreSites', async ({ siteRestoreInfos, dryRun, confirmDangerous, customHeaders }) => {
        const targetSites = siteRestoreInfos.map((s) => s.siteId).join(', ');
        if (dryRun) {
...
//# sourceMappingURL=restoreSites.js.map
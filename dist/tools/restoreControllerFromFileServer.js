...
export function registerRestoreControllerFromFileServerTool(server, client) {
    ...
    server.registerTool('restoreControllerFromFileServer', {
        description: 'Restore controller configuration from a backup file stored on an external file server (FTP/SFTP). This will overwrite current controller settings. Use getRestoreResult to poll the restore status. REQUIRES explicit confirmDangerous: true — a second confirmation step is enforced because this action is irreversible.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapToolHandler('restoreControllerFromFileServer', async ({ serverConfig, filePath, skipDevice, dryRun, confirmDangerous, customHeaders }) => {
        if (dryRun) {
...
//# sourceMappingURL=restoreControllerFromFileServer.js.map
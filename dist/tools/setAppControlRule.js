...
export function registerSetAppControlRuleTool(server, client) {
    server.registerTool('setAppControlRule', {
        description: 'Create or update an application control rule with dry-run preview support. Create uses the Omada AddRuleEntity schema; update uses the Omada EditRuleEntity schema.',
        inputSchema: setAppControlRuleInputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapMutationToolHandler('setAppControlRule', ({ ruleId, siteId }, result, mode) => ({
...
//# sourceMappingURL=setAppControlRule.js.map
...
import { buildEffectiveReservation, dhcpReservationPayloadSchema, findLanNetworkById, findReservationByIp, findReservationByMac, getAllDhcpReservations, validateReservationIpAgainstNetwork, } from './dhcpReservationShared.js';
export function registerUpdateDhcpReservationTool(server, client) {
    ...
    server.registerTool('updateDhcpReservation', {
        description: 'Update an existing DHCP reservation after validating the target LAN network and reservation IP.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapMutationToolHandler('updateDhcpReservation', ({ reservationMac, siteId }, result, mode) => ({
        ...
        if (effectiveReservation.ip) {
            ...
            if (conflictingReservation && !effectiveReservation.confirmConflict) {
                throw new Error(`IP ${effectiveReservation.ip} is already reserved by ${conflictingReservation.mac}. Set confirmConflict to true only if you intend to override the controller conflict warning.`);
            }
        }
        if (dryRun) {
            return {
...
//# sourceMappingURL=updateDhcpReservation.js.map
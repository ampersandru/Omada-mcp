...
function parseApPayload(configType, payload) {
    switch (configType) {
        case 'service-config':
...
function hasReadback(configType) {
    return !['service-config', 'wlan-group', 'channel-config'].includes(configType);
}
async function validateApConfigCapability(client, configType, apMac, payload, siteId, customHeaders) {
    if (configType !== 'channel-config') {
        return;
    }
    const availableChannels = (await client.getSitesApsAvailableChannel(apMac, siteId, customHeaders));
...
async function getDeviceSummary(client, apMac, siteId, customHeaders) {
...
async function getCurrentApConfig(client, configType, apMac, siteId, customHeaders) {
    switch (configType) {
        case 'general-config':
...
async function applyApConfig(client, configType, apMac, payload, siteId, customHeaders) {
    switch (configType) {
        case 'general-config':
...
export function registerSetApConfigTool(server, client) {
    server.registerTool('setApConfig', {
        description: 'Update a documented Omada AP configuration family with dry-run support. Covers general, IP, IPv6, QoS, radio, service, load-balance, OFDMA, trunk, bridge, WLAN group, port, channel, AFC, and antenna settings.',
        inputSchema: inputSchema.shape,
        annotations: {
            destructiveHint: true,
        },
    }, wrapMutationToolHandler('setApConfig', ({ apMac, configType, siteId }, result, mode) => ({
        ...
        const resolvedConfigType = configType;
...
//# sourceMappingURL=setApConfig.js.map